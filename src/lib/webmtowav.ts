// Define a type for the target array parameter
type TargetArray = Uint8Array;

// Define a type for the audio buffer parameter
type AudioBuffer = {
  length: number;
...
// Define a type for the context options parameter
type ContextOptions = {
  // Define the properties of the context options here
  // For example:
  sampleRate?: number;
  // Add other properties as needed
};

// Convert the JavaScript functions to TypeScript with correct typing
...
function _writeAudioBufferToArray(
  ...
  let channelData: Float32Array, sample: number;
...
async function _getAudioBuffer(
  ...
  let blob: Blob;
  if (!(blobData instanceof Blob)) blob = new Blob([...blobData]);
  else blob = blobData;
